...
function AutoSizedImage({ uri }) {
  ...
  useEffect(() => {
    Image.getSize(
      uri,
      (w, h) => {
        const scaleFactor = (width * 0.9) / w;
        const imageHeight = h * scaleFactor;
        setDimensions({ width: width * 0.9, height: imageHeight });
      },
...
export default function ProductPage() {
  ...
  const [innerButtonY, setInnerButtonY] = useState(0);
  ...
  useEffect(() => {
    if (productId) {
      fetch(
        `https://muditam-app-backend-ca1c8b03db09.herokuapp.com/api/shopify/product/${productId}`
      )
        .then((res) => res.json())
        .then((data) => {
          setProductData(data);
  ...
  const getItemQuantity = (selectedVariantId) => {
    const item = cartItems.find(
      (cartItem) => cartItem.id === selectedVariantId
    );
  ...
  const handleDecrement = (item) => {
    decrementItem(item.id); // You already handle 0 qty in CartContext
  };
  ...
  const displayedImages =
    selectedVariantId && selectedVariant?.image
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView
        ...
        onScroll={() => {
          if (addToCartRef.current) {
            addToCartRef.current.measure(
              (x, y, width, height, pageX, pageY) => {
                const screenHeight = Dimensions.get("window").height;
                const topSafeArea = insets.top || 0;
                const bottomSafeArea = insets.bottom || 0;


                const isVisible =
                  pageY >= topSafeArea &&
                  pageY + height <= screenHeight - bottomSafeArea;
      ...
      >
        ...
        <View style={[styles.headerContainer]}>
          ...
          >
            ...
            {getItemQuantity(selectedVariantId) > 0 && (
              <View style={styles.cartBadge}>
                <Text style={styles.cartBadgeText}>
                  {getItemQuantity(selectedVariantId)}
                </Text>
              </View>
            )}
          </TouchableOpacity>
        ...
        <View style={styles.details}>
          ...
          <View style={styles.ratingRow}>
            ...
            <Text style={styles.ratingText}>
              Rated
              <Text style={{ fontWeight: "bold" }}> 4.8/5</Text> based on 1200+
              Reviews
            </Text>
          </View>
        ...
        <View style={styles.variantsContainer}>
          {variants.slice(0, 3).map((variant) => {
            const packText =
              variant.metafields?.custom?.pack_usage || variant.title;
            ...
            return (
              ...
              >
                ...
                {variant.compare_at_price && (
                  <Text style={styles.variantCompare}>
      ...
      {isPriceVisible && (
        <View style={styles.priceContainer}>
          <View>
            {selectedVariant && (
              <Text>₹ {parseFloat(selectedVariant.price) * quantity}</Text>
            )}
          ...
          <View
            onLayout={(event) => {
              ...
              setInnerButtonY(layout.y);
            }}
          >
           
            {getItemQuantity(selectedVariant?.id) > 0 ? (
              <View style={styles.counterContainer}>
                <TouchableOpacity
                  ...
                  onPress={() =>
                    handleDecrement({
                      ...selectedVariant,
                      title: `${productData.title} - ${selectedVariant.title}`,
                      image: selectedVariant.image || productData.images?.[0],
                ...
                <Text style={styles.counterValue}>
                  {getItemQuantity(selectedVariant?.id)}
                </Text>
              ...
              <TouchableOpacity
                ...
                onPress={() =>
                  addToCart({
                    ...
                    first_variant_id: selectedVariant.id,
                    title: `${productData.title} - ${selectedVariant.title}`,
                    image: selectedVariant.image || productData.images?.[0],
                    price: parseFloat(selectedVariant.price),
                    quantity: 1,
